...
# __file__ es tests/pact_tests/test_flask_pact.py
...
PACT_FILE = str(_PROJECT_ROOT / "pacts" / "DisponibilidadClient-FlaskDisponibilidadAPI.json")

# ── Creación del objeto Pact ───────────────────────────────────────────────────

# Pact("consumidor", "proveedor") crea el objeto que acumula las interacciones.
# "DisponibilidadClient" es el nombre del servicio consumidor (el cliente HTTP).
# "FlaskDisponibilidadAPI" es el nombre del servicio proveedor (la API Flask).
# Estos nombres se usan para nombrar el archivo JSON del contrato.
pact = Pact("DisponibilidadClient", "FlaskDisponibilidadAPI")
...
def pact_mock():
    ...
    # ── Interacción 1: GET /disponibilidad/ ───────────────────────────────────
    (
        pact.upon_receiving("GET /disponibilidad/ returns a list")
        .given("at least one disponibilidad exists")        # estado previo del proveedor
        .with_request("GET", "/disponibilidad/")      # método y ruta de la petición
        .will_respond_with(200)                        # código de estado esperado
        .with_body(
            match.each_like(
                {
                    "disponibilidadId": match.integer(1),
                    "bookId": match.integer(1),
                    "available": match.boolean(True),
                    "lastUpdated": match.string("2026-05-31T12:00:00"),
                }
            ),
    ...
    # ── Interacción 2: POST /disponibilidad/7 ──────────────────────────────────
    #
    # El consumidor dice: "cuando envíe POST con body {"bookId": 20, "available": false},
    # espero un 201 con el objeto creado (id 7, bookId 28, available false)."
    #
    # Nota: .with_body() ANTES de .will_respond_with() → aplica al REQUEST.
    #       .with_body() DESPUÉS de .will_respond_with() → aplica al RESPONSE.
    (
        pact.upon_receiving("POST /disponibilidad/7 creates a new disponibilidad")
        .given("any state")
        .with_request("POST", "/disponibilidad/7")
        .with_body({"bookId": 7, "available": False, "reason": "LOANED"}, content_type="application/json")  # body del request
        .will_respond_with(201)
        .with_body(                                                          # body del response
            {"disponibilidadId": match.integer(7), "bookId": 7, "available": False, "reason": match.string("LOANED"), "lastUpdated": match.string("2026-05-31T12:00:00")},
            content_type="application/json",
        )
        .with_header("Content-Type", "application/json")
    )

    # ── Interacción 3: GET /flask/disponibilidad/3 ────────────────────────────────
    #
    ...
    (
        pact.upon_receiving("GET /disponibilidad/3 returns the disponibilidad")
        .given("a disponibilidad with bookId 3 exists")
        .with_request("GET", "/disponibilidad/3")
        .will_respond_with(200)
        .with_body(
            {"disponibilidadId": match.integer(6), "bookId": 3, "available": match.boolean(False), "lastUpdated": match.string("2026-05-31T12:00:00")},
            content_type="application/json",
        )
        .with_header("Content-Type", "application/json")
    )

    # ── Interacción 4: GET /disponibilidad/9999 (404) ─────────────────────────
    #
    # match.string("not found") → el mensaje puede ser cualquier string.
    (
        pact.upon_receiving("GET /disponibilidad/9999 returns 404")
        .given("no disponibilidad with bookId 9999 exists")
        .with_request("GET", "/disponibilidad/9999")
...
class TestFlaskConsumer:
    ...
    def test_list_disponibilidad(self, pact_mock):
        ...
        r = requests.get(f"{pact_mock.url}/disponibilidad/")

        # Verificamos que el consumidor recibe lo que espera.
        assert r.status_code == 200
        assert isinstance(r.json(), list)  # el consumidor sabe que es un array

    def test_create_disponibilidad(self, pact_mock):
        # POST con el body exacto que el consumidor enviará en producción.
        r = requests.post(
            f"{pact_mock.url}/disponibilidad/7",
            json={"bookId": 7, "available": False, "reason": "LOANED"},
        )
        assert r.status_code == 201
        assert r.json()["bookId"] == 7  # el id devuelto debe coincidir
        assert r.json()["available"] is False # el consumidor verifica que el campo "available" es False
        assert r.json()["reason"] == "LOANED" # el consumidor verifica que el campo "reason" es "LOANED"  

    def test_get_disponibilidad_by_id(self, pact_mock):
        r = requests.get(f"{pact_mock.url}/disponibilidad/3")
        assert r.status_code == 200
        # El consumidor verifica que el id devuelto es el que pidió.
        assert r.json()["bookId"] == 3

    def test_get_disponibilidad_not_found(self, pact_mock):
        r = requests.get(f"{pact_mock.url}/disponibilidad/9999")
...
def flask_provider_url():
    """
    Fixture que levanta la app Flask REAL de disponibilidad en PROVIDER_PORT.

    Aquí usamos el servicio de disponibilidad con sus datos en memoria ya
    inicializados en `tarea4/disponibilidad/app.py`.
    """

    # No se necesita poblar datos aquí: `disponibilidad_list` ya contiene
    # registros iniciales en memoria desde el módulo `disponibilidad.app`.
    server = make_server("localhost", PROVIDER_PORT, disponibilidad_app)
    ...
    # Debug: comprobamos rápidamente que el servidor responde.
    import requests
    try:
        r = requests.get(f"http://localhost:{PROVIDER_PORT}/disponibilidad/")
        print(f"DEBUG: Server responds with {r.status_code}, headers: {r.headers.get('content-type')}")
        print(f"DEBUG: Body: {r.text[:200]}")
    except Exception as e:
        print(f"DEBUG: Server error: {e}")

    # Entregamos la URL del proveedor al test; el teardown detendrá el servidor.
...
class TestFlaskProvider:
    ...
    def test_provider_honours_pact(self, flask_provider_url):
        # Verifier("FlaskDisponibilidadAPI") crea el verificador para el proveedor.
        # El nombre debe coincidir exactamente con el nombre del proveedor
        # usado en Pact("DisponibilidadClient", "FlaskDisponibilidadAPI").
        #
        ...
        verifier = (
            Verifier("FlaskDisponibilidadAPI")
