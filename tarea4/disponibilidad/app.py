...
def rabbitmq():
    ...
    return jsonify("Disponibilidad enviada a RabbitMQ"), 200
...
def add_disponibilidad(book_id: int):
    try:
        ...
        # Assign the new id before creating the Pydantic model so validation
        # (which requires `disponibilidadId`) succeeds.
        new_id = next_disponibilidad_id_get()
        new_disponibilidad = Disponibilidad(disponibilidadId=new_id, **loaded_data)
